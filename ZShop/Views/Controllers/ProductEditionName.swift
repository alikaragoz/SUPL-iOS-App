...
internal final class ProductEditionName: UIViewController {
  private let viewModel: ProductEditionNameViewModelType = ProductEditionNameViewModel()
  ...
  let titleLabel = UILabel().then {
    ...
    $0.text = NSLocalizedString(
      "edit_product.name.title",
      value: "What's the name of the product?",
      comment: "Title of the page where we ask the user for the name of the product.")
  }

  let saveButton = FloatButton().then {
    $0.translatesAutoresizingMaskIntoConstraints = false
    let saveTitle = NSLocalizedString(
      "edit_product.name.save_button.title",
      value: "Save",
      comment: "Title of the button to save the edited name in the edit name page")
  ...
  internal static func configuredWith(name: String) -> ProductEditionName {
    let vc = ProductEditionName(nibName: nil, bundle: nil)
  ...
  private func dismiss(withConfirm confirmation: Bool) {
    ...
    if confirmation == false {
      ...
      let message = NSLocalizedString(
        "edit_product.name.exit_confirm.title",
        value: "Are you sure you want to quit? You will lose all your edits on the name.",
        comment: "Title of alert to confirm if the user wants to quit the product name edition.")

      let quitActionTitle = NSLocalizedString(
        "edit_product.name.exit_confirm.lose_edits_title",
        value: "Lose Edits",
        comment: "Title of the button to confirm the exit.")

      let cancelActionTitle = NSLocalizedString(
        "edit_product.name.exit_confirm.cancel_title",
        value: "Cancel",
